...
from backend.tools.keyboard import system_type_text, press_key
...
@patch("backend.tools.keyboard.user32.PostMessageW", return_value=1)
@patch("backend.tools.keyboard.user32.GetForegroundWindow", return_value=0)
def test_type_text(mock_fg, mock_pm, mock_write):
    res = system_type_text("hello")
    assert res["success"] is True
    mock_write.assert_called_once_with("hello", interval=0.04)
